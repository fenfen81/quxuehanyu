      ...
      {view === 'learning' && (
        <>
          ...
          <div className="flex gap-2 justify-center items-center flex-wrap">
            ...
            <button onClick={()=>{setShowExampleEn(p=>!p);sfx.play('click')}} className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${showExampleEn?'bg-indigo-50 text-indigo-600 border-indigo-200':'bg-white text-slate-400 border-slate-200'}`}>{showExampleEn?'🔤':'🙈'}{tt('words_example_en_toggle')}</button>
              ...
              <div
                ...
                <div className="relative w-full transition-all duration-500" style={{transformStyle:'preserve-3d',transform:flipped?'rotateY(180deg)':'rotateY(0deg)',minHeight:'280px'}}>
                  ...
                  <div className="absolute inset-0 rounded-2xl bg-white border border-slate-200 shadow-md flex flex-col items-center justify-center gap-3 p-6 sm:p-8" style={{backfaceVisibility:'hidden'}}>
                    ...
                    <div className="flex items-center gap-2"><div className="text-4xl sm:text-5xl font-black text-slate-800 font-kai">{current.hanzi}</div><SpeakBtn text={current.hanzi} wordId={current.id} className="w-9 h-9 sm:w-10 sm:h-10 text-lg bg-indigo-50 text-indigo-600"/></div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mt-1 px-4 py-3 rounded-xl bg-white/60 border border-indigo-100 max-w-[95%]">
                          ...
                          {showExampleEn && <div className="text-sm sm:text-base text-slate-400 mt-1">{ex.en}</div>}
                        </div>
                      )
                    })()}
          ...
          {mode==='quiz' && current && !showResult && (
            <div className="space-y-4">
              <div className="rounded-2xl bg-white border border-slate-200 shadow-md p-6 flex flex-col items-center gap-3 min-h-[180px] justify-center relative">
                ...
                <div className="flex items-center gap-2"><div className="text-4xl font-black text-slate-800 font-kai">{current.hanzi}</div><SpeakBtn text={current.hanzi} wordId={current.id} className="w-8 h-8 text-lg bg-indigo-50 text-indigo-600"/></div>
            ...
            <div className="space-y-5">
              <div className="rounded-2xl overflow-hidden border border-slate-200 shadow-md" style={{background:typedCorrect===false?'linear-gradient(135deg,#fef2f2,#fff1f2)':typedCorrect===true?'linear-gradient(135deg,#f0fdf4,#f0fff4)':'white'}}>
                ...
                <div className="px-6 pb-7 pt-5 space-y-4">
                  ...
                  {typedCorrect!==null && (<div className="text-center">{typedCorrect?(<div className="text-green-600 font-bold">✅ {tt('words_type_correct')} <span className="font-kai">{current.hanzi}</span></div>):(<div className="text-red-500 font-medium">❌ {tt('words_type_wrong')}<span className="font-bold text-lg ml-1 font-kai">{current.hanzi}</span></div>)}</div>)}
                </div>
              </div>
