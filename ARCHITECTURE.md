...
9. **onAuthStateChange 误重置页面**：`supabase.auth.onAuthStateChange` 回调里曾写 `if (s) setPage('home')`，但 Supabase 在标签页失焦再聚焦（`visibilitychange`）/ 网络恢复时会自动刷新 token 并触发该回调，导致用户切窗口/开文件夹就被弹回首页。**最终修复：彻底移除该回调里的 `setPage` 调用**，仅保留 `setSession(s)`；登录成功后进入首页由 `page` 的 `useState` 默认值 `'home'` 保证，无需在 auth 事件中干预（仅限制为 `SIGNED_IN` 仍不稳定，故彻底删除）。
