#!/usr/bin/env node
// gen_hj2a_audio.cjs — 为 content.ts 中《汉语教程》第二册（上）(hanyu-jiaocheng-2a)
// 生成整句 + 分段语音，逻辑与 pipeline/06_audio.cjs 一致（XiaoxiaoNeural / 24k MP3）。
// 直接从 content.ts 提取 hanyu-jiaocheng-2a 对象，保证语音与「表格拼音分词」完全对应。
// 幂等：public/audio 中已存在且 >1KB 的跳过；可重复跑补齐（断点续跑）。
// 用法（在本机，需联网 Microsoft Edge TTS）：
//   node gen_hj2a_audio.cjs
const fs = require('fs')
...
const BOOK_ID = 'hanyu-jiaocheng-2a'

// ---- 从 content.ts 提取 hanyu-jiaocheng-2a 对象 ----
...
if (process.argv.includes('--count')) {
  ...
  for (const t of tasks) {
    const m = t.file.match(/audio\/(hj2a-l\d+)-/)
