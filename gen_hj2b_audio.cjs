#!/usr/bin/env node
// gen_hj2b_audio.cjs — 为 content.ts 中《汉语教程》第二册（下）(hanyu-jiaocheng-2b)
// 生成整句 + 分段语音，逻辑与 gen_hj2a_audio.cjs / pipeline/06_audio.cjs 一致（XiaoxiaoNeural / 24k MP3）。
// 直接从 content.ts 提取 hanyu-jiaocheng-2b 对象，保证语音与「表格拼音分词」完全对应。
// 幂等：public/audio 中已存在且 >1KB 的跳过；可重复跑补齐（断点续跑）。
// 用法（本机或沙盒，需联网 Microsoft Edge TTS）：
//   node gen_hj2b_audio.cjs
//   CONCURRENCY=8 node gen_hj2b_audio.cjs
const fs = require('fs')
...
const BOOK_ID = 'hanyu-jiaocheng-2b'

// ---- 从 content.ts 提取 hanyu-jiaocheng-2b 对象 ----
...
if (process.argv.includes('--count')) {
  ...
  for (const t of tasks) {
    const m = t.file.match(/audio\/(hj2b-l\d+)-/)
