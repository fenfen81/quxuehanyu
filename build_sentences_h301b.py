...
# -*- coding: utf-8 -*-
"""
把「课文原文及翻译.xlsx」的句子导入 content.ts（句子练习模块）。
复用 build_sentences_2b.py / build_sentences_h301a.py 的拼音逐音节对齐算法。
规则：
  - 拼音：直接使用 xlsx「拼音」列（分词形式）。
  - 分词(split)：由拼音列逐音节对齐得到中文词边界。
  - 英文：直接使用 xlsx「地道英文」列（句子级）。
  - dict 逐词英文：优先用「生词表.xlsx」的本课生词英文（若有），
    再叠加 03_enrich.cjs 的 COMMON 词表与 SUPP 常用词；无英文的词不写入 dict。
用法：复制到 app/ 根目录后改下方常量，然后
  python build_sentences_xxx.py        # dry-run，写 _sentences_xxx.json
  python build_sentences_xxx.py --emit # 写入 content.ts
"""
import re, json, sys, os

# ===== 按教材修改以下常量 =====
XLSX = r"E:/我的教学资料/趣学汉语网站制作相关文件/我制作趣学汉语网站所需的一些教材材料/《汉语会话301句》下册 的相关文件/《汉语会话301句》下册 课文原文及翻译.xlsx"
VOCAB_XLSX = r"E:/我的教学资料/趣学汉语网站制作相关文件/我制作趣学汉语网站所需的一些教材材料/《汉语会话301句》下册 的相关文件/汉语会话301句下册-生词表.xlsx"
BOOK_ID = "hanyu-huihua-301-1b"
SID_PREFIX = "h301b"
CATEGORY_ID = "oral"
TITLE = "《汉语会话301句》下册"
TITLE_EN = "Conversational Chinese 301 Vol.2"
LEVEL = "初级"
INSERT_MARKER = ""                       # 可选：插在该书 id 之后；留空=插到 textbooks 数组末尾
# ==============================

APP = os.environ.get("QX_APP") or os.path.dirname(os.path.abspath(__file__))
...
def load_vocab_en():
    ...
    for row in data.iter(W + 'row'):
        ...
        if len(r) >= 2 and r[0].isdigit() and re.search(r'[\u4e00-\u9fff]', r[1]):
            if len(r) >= 5 and r[4]:
                out[cur][r[1]] = r[4]
    return out
    ...
    for r in rows:
        ...
        if len(r) >= 4 and r[0].isdigit() and HANZI.search(r[2]):
            if cur_text is None:
                ...
                cur["texts"].append(cur_text)
            py = r[1]
            # ── 精确修正（原文+拼音同时匹配才替换，防误伤其他句子）──
            # h301a 遗留示例：原文「在。」被误标成 Zàijiā。
            if r[2] == "在。" and re.match(r'^Zàijiā[。\s]', py):
                py = "Zài。"
            # h301b：连写多音节 / 表格拼音笔误 / 阿拉伯数字配汉字读音
            FIX = [
                (r"这个小东西多可爱", "kěài", "kě ài"),                          # 连写两音节被误算成 1 音节
                (r"你得注意安全", "zhùyìānquán", "zhù yì ān quán"),              # 缺分词空格（同 hj2b 笔误）
                (r"71 米", "qīshí yī mǐ", "71 mǐ"),                              # 阿拉伯数字+汉字读音
                (r"14 米", "shísì mǐ", "14 mǐ"),
                (r"7 米", "qī mǐ", "7 mǐ"),
                (r"2012 年 9 月", "Èr líng yī èr nián jiǔ yuè", "2012 nián 9 yuè"),
            ]
            for pat, before, after in FIX:
                if re.search(pat, r[2]) and before in py:
                    py = py.replace(before, after)
...
def main():
    ...
    for les in lessons:
        ...
        for ti, tx in enumerate(les["texts"], 1):
            ...
            for si, s in enumerate(tx["sentences"], 1):
                ...
                total_s += 1
                total_dk += len(dct)
            out_texts.append({"id": f"{SID_PREFIX}-l{les['num']}-t{ti}", "label": tx["label"], "sentences": out_sents})
    ...
    with open(os.path.join(APP, f"_sentences_{SID_PREFIX}.json"), "w", encoding="utf-8") as f:
        json.dump({"lessons": out_lessons}, f, ensure_ascii=False, indent=1)
    print(f"[info] 已写 _sentences_{SID_PREFIX}.json")
    if DRY_RUN:
...
def find_book_close(marker_id):
    """括号配对：定位指定书 id 对应对象开括号，返回其闭合 } 的下标（rfind 会误判，禁用）。
    跳过字符串内括号（dict 英文可能含 [ ] { }）。"""
    ts = open(CONTENT, encoding="utf-8").read()
    i = ts.index(f'"id": "{marker_id}"')
    ...
    for j in range(open_idx, len(ts)):
        ...
        if in_str:
            ...
            if ch == '"':
                ...
                if depth == 0:
                    return ts, j
    raise RuntimeError(f"未找到 {marker_id} 的闭合括号")
...
def emit(out_lessons):
    ...
    ts = open(CONTENT, encoding="utf-8").read()
    if INSERT_MARKER:
        # 插在指定书之后：该书闭合 } 与下一本 {（或数组 ]）之间
        ts, close_idx = find_book_close(INSERT_MARKER)
        new_ts = ts[:close_idx + 1] + ",\n" + obj_indented + ts[close_idx + 1:]
    else:
        # 默认插到 textbooks 数组末尾：定位数组字面量整体 [ ... ]（括号配对），在闭合 ] 前插入
        # ⚠️ 必须从 `= [` 找数组开括号；直接 ts.index("[", start) 会误匹配类型注解 Textbook[]
    ...
    print(f"[emit] 已插入 {BOOK_ID}（textbooks 数组末尾，括号配对定位）")
